      ...
      <section className="bg-ink-900 py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-start gap-6">
            <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${prog.color} flex items-center justify-center text-4xl flex-shrink-0`}>
              {prog.icon}
            </div>
            <div>
              ...
              <div>
                ...
                <ul className="space-y-3">
                  ...
                  <li className="flex gap-3 text-ink-700">
                    <span className="text-gold-500 text-lg leading-tight mt-0.5">✓</span>
                    <span>Dibiayai sepenuhnya oleh PTPK (tiada beban kewangan semasa pengajian)</span>
                  </li>
                  <li className="flex gap-3 text-ink-700">
                    <span className="text-gold-500 text-lg leading-tight mt-0.5">✓</span>
                    <span>Sijil diiktiraf JPK K16005 — diterima pakai oleh majikan seluruh Malaysia</span>
                  </li>
                </ul>
              </div>
              ...
              {!isComingSoon && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {prog.careers.map(c => (
                      <span key={c} className="px-4 py-2 bg-ink-50 text-ink-700 rounded-xl border border-ink-100 text-sm font-medium">
                        {c}
                      </span>
                    ))}
                  </div>
              ...
              <div className="sticky top-6 space-y-4">
                <div className="card p-6 border-2 border-gold-400">
                  <h3 className="font-bold text-ink-900 text-lg mb-1">
                    {isComingSoon ? 'Daftar Minat' : 'Daftar Sekarang'}
                  </h3>
                  <p className="text-sm text-ink-500 mb-4">
                    {isComingSoon
                      ? 'Tinggalkan nama anda — kami akan hubungi bila program ini dibuka.'
                      : 'Hubungi kaunselor kami untuk maklumat lanjut dan borang pendaftaran.'}
                  </p>
                  <a
                  ...
                  <div className="mt-4 pt-4 border-t border-ink-100 space-y-2">
                    <a href={`tel:${CONTACT.phone1.replace(/\s/g, '')}`} className="flex items-center gap-2 text-sm text-ink-600 hover:text-gold-500 transition-colors">
                      <span>📞</span>
                      <span>{CONTACT.phone1}</span>
                    </a>
                    <a href={`tel:${CONTACT.phone2.replace(/\s/g, '')}`} className="flex items-center gap-2 text-sm text-ink-600 hover:text-gold-500 transition-colors">
                      <span>📱</span>
                ...
                {prog.brochure && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="btn-outline-gold w-full text-center block py-3"
                  >
                    Muat Turun Brosur PDF
                  </a>
                )}
                ...
                <div className="card p-5 bg-ink-50 border-0">
                  ...
                  <ul className="space-y-2">
                    {PROGRAMS.filter(p => p.slug !== prog.slug).slice(0, 4).map(p => (
                      <li key={p.slug}>
                        <Link href={`/program/${p.slug}`} className="flex items-center gap-2 text-sm text-ink-600 hover:text-gold-500 transition-colors">
                          <span>{p.icon}</span>
                  ...
                  <Link href="/program" className="text-xs text-gold-500 hover:text-gold-600 font-medium mt-3 inline-block">
                    Lihat semua program →
                  </Link>
                </div>
              </div>
      ...
      <section className="py-14 bg-ink-900">
        <div className="max-w-2xl mx-auto px-4 text-center">
          <h2 className="text-2xl font-bold text-white mb-3">
            {isComingSoon ? `Berminat dengan program ${prog.name}?` : `Bersedia untuk mendaftar ${prog.name}?`}
          </h2>
          <p className="text-ink-300 mb-6">
            {isComingSoon
              ? 'Program ini akan dibuka tidak lama lagi. Daftar minat sekarang dan kami akan maklumkan anda terus.'
              : 'Pendaftaran terbuka sekarang. Hubungi kami dan kami akan bantu proses pendaftaran anda dalam masa 24 jam.'}
          </p>
          <a
            href={CONTACT.whatsapp}
            target="_blank"
            rel="noopener noreferrer"
            className="btn-gold px-10 py-3.5 inline-block"
          >
            {isComingSoon ? 'Daftar Minat Percuma' : 'Daftar Program Ini'}
          </a>
        </div>
      </section>
    </>
